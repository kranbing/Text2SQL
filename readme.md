...
- 暂仅支持 MySQL，支持自定义的模型调用与 RAG 检索（支持 txt/md/csv/xlsx/xls/docx）
...
3) 快速验证
   ...
   - 启动 CLI：`python text2sql/cli.py`
   - 首次连接可用 `-s` 显示并生成 `text2sql/db_info/<db>.json`
   - 保存结果：使用 `-j` 或 `-c`，结果文件位于 `text2sql/results/`，JSON 为缩进+换行的可读格式
